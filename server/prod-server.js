...
if(fs.existsSync(distPath)){
  ...
  app.get('/', (req,res)=>res.send('Run npm run build first. Dist not found.'));
}
...
wss.on('connection', (ws)=>{
  ...
  ws.on('close', ()=>{
    console.log(`[WW2] Player left: ${ws._id}`);
    players.delete(ws._id);
    broadcast({ t: 'player-left', id: ws._id });
  });
});
...
httpServer.listen(port, host, ()=>{
  console.log(`\n[WW2 Arena] Production server listening on http://${host}:${port}`);
  console.log(`[WW2 Arena] Players join via http://${host}:${port} on same network`);
  console.log(`[WW2 Arena] WebSocket: ws://${host}:${port}/ws\n`);
});
